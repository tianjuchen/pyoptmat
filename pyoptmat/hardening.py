...
class NNKinematicHardeningModel(KinematicHardeningModel):
    ...
    def __init__(self, alpha, weights1, biases1, weights2, biases2, weights3, biases3):
        super().__init__()
        self.alpha = alpha
    ...
    def model(self, T):
        return torch.nn.Sequential(
            Lin(self.weights1(T), self.biases1(T)),
            nn.GELU(),
            Lin(self.weights2(T), self.biases2(T)),
            nn.GELU(),
            Lin(self.weights3(T), self.biases3(T)),
            )
    ...
    def model_derivative(self, x, T):
        return jacrev(self.model(T))(x).diagonal(dim1=0,dim2=2).transpose(0,-1)
