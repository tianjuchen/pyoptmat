...
class PolynominalRegression:
    ...
    def train_step(self, xs, ys):
        ...
        for i in range(xs.shape[-1]):
            x_ = Variable(xs[:, i].to(torch.float64), requires_grad=True)
            y_ = Variable(
                ys[:, i].unsqueeze(1)
            )  # unsqueeze to match dimensions with y_pred later
    ...
    def coeff_coeff(self, niter, xs, ys):
        ...
        for epoch in t:
            running_loss = self.train_step(xs, ys)
            t.set_description("Loss: %3.2e" % running_loss)
            # if epoch % 100 == 99:
            # print(f"Epoch: {epoch + 1:04}/2000 Loss: {running_loss:.5e}")
        coeffs = []
        for param in self.model.parameters():
            for elem in param.detach().numpy().flatten():
                coeffs.append(elem.item())
        return coeffs

    def expolate_func(self, niter, xs, ys, x, degree):
        coeffs = self.coeff_coeff(niter, xs, ys)
        y = 0.0
        pow_v = degree
        for coeff in coeffs:
            y += coeff * x**pow_v
...
class PolynominalMultipleRegression:
    def __init__(self, model, degree):
        self.degree = degree
        self.model = model
        self.optimizer = torch.optim.Adam(
            model.parameters(), lr=0.1, weight_decay=0.0001
        )
        self.criterion = nn.MSELoss(reduction="sum")

    def train_step(self, xs, ys):
        x_ = Variable(xs.to(torch.float64), requires_grad=True)
        y_ = Variable(
            ys.unsqueeze(1)
        )  # unsqueeze to match dimensions with y_pred later
    ...
    def coeff_coeff(self, niter, xs, ys):
        ...
        return np.array(coeffs).reshape(
            int(len(coeffs) / (self.degree + 1)), self.degree + 1
        )

    def expolate_func(self, niter, xs, ys, x):
        coeffs = self.coeff_coeff(niter, xs, ys)
        ...
        for i in range(x.shape[-1]):
            ...
            for coeff in curr_coeffs:
                curr_y += coeff * x[:, i] ** pow_v
                pow_v += -1
            y[:, i] = curr_y
        return y


class HierarchicalPolyModel(PyroModule):
    """
    Wrap a material model to provide a hierarchical :py:mod:`pyro` statistical
    model
    ...
    def forward(
        ...
        with pyro.plate("trials", exp_data.shape[2]):
            ...
            if (exp_types[0] == 2) or (exp_types[0] == 4):
                poly_model = PolynomialModel(self.degree)
                tmodel = PolynominalMultipleRegression(poly_model, self.degree)
...
class LogModel(nn.Module):
    ...
    def forward(self, x):
        return self.b + torch.log(x + 1.0) * self.a


class ExponentialMultipleRegression:
    def __init__(self, model):
        self.model = model
        self.optimizer = torch.optim.Adam(
            model.parameters(), lr=0.1, weight_decay=0.0001
    ...
    def train_step(self, xs, ys):
        x_ = Variable(xs.to(torch.float64), requires_grad=True)
        y_ = Variable(ys)  # unsqueeze to match dimensions with y_pred later
        preds = self.model(x_)
        loss = self.criterion(preds, y_)
        loss.backward()
        self.optimizer.step()
        self.optimizer.zero_grad()
        return loss
    ...
    def coeff_coeff(self, niter, xs, ys):
        ...
        return torch.tensor(coeffs).reshape(int(len(coeffs) / 2), 2)

    def expolate_func(self, niter, xs, ys, x):
        coeffs = self.coeff_coeff(niter, xs, ys)
        y = torch.zeros_like(x)
        for i in range(x.shape[-1]):
            y[:, i] = coeffs[i, 1] + torch.log(x[:, i] + 1.0) * coeffs[i, 0]
        return y


class HierarchicalLogModel(PyroModule):
    """
    Wrap a material model to provide a hierarchical :py:mod:`pyro` statistical
    model
