...
if __name__ == "__main__":
  ...
  scale = 0.15
  nsamples = 2 # at each strain rate
  ...
  range_min = [torch.tensor(0.0, device = device) for i in range(len(names))]
  range_max = [torch.tensor(1.0, device = device) for i in range(len(names))]
  
  print("Initial parameter values:")
  print("\tloc loc\t\tloc scale\tscale scale")
  for n, llp, lsp, sp in zip(names, loc_loc_priors, range_min, 
      range_max):
    print("%s:\t%3.2f\t\t%3.2f\t\t%3.2f" % (n, llp, lsp, sp))
  print("")

  # 3) Create the actual model
  model = optimize.MLEOptim(make, names, loc_loc_priors,
      loc_scale_priors, scale_scale_priors, eps, range_min, range_max).to(device)
