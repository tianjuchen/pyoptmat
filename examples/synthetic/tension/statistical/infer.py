...
>>>>>>> e490022e7cf6ecbdb13c92232b8bafeaf445fce7
  ...
  for i, ax in enumerate(axs.reshape(-1)):
    site = sites[i]
    for j, ax_2 in enumerate(axs.reshape(-1)):
      site_2 = sites[j]
      sns.kdeplot(predictive_svi[site].detach().numpy(), predictive_svi[site_2].detach().numpy(), 
                  ax=axs[0], label="SVI (Multivariate Normal)")
      axs[i].set(xlabel=site, ylabel=site_2)
