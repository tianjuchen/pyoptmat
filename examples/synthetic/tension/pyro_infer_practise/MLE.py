...
if __name__ == "__main__":
  ...
  mean_n, mean_eta, mean_s0, mean_R, mean_d = map(torch.tensor, (ra.uniform(0.25,0.75), 
                                                  ra.uniform(0.25,0.75), ra.uniform(0.25,0.75), 
                                                  ra.uniform(0.25,0.75), ra.uniform(0.25,0.75)))

  def model(times, strains, temperatures, true_stresses):
    n_loc = pyro.param("n_loc", mean_n)
    eta_loc = pyro.param("eta_loc", mean_eta)
    s0_loc = pyro.param("s0_loc", mean_s0)
    R_loc = pyro.param("R_loc", mean_R)
    d_loc = pyro.param("d_loc", mean_d)
    ...
    with pyro.plate("trials", times.shape[1]):
      n = pyro.param("n", mean_n)
      eta = pyro.param("eta", mean_eta)
      s0 = pyro.param("s0", mean_s0)
      R = pyro.param("R", mean_R)
      d = pyro.param("d", mean_d)
