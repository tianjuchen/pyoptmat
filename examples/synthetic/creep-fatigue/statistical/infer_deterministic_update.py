...
if __name__ == "__main__":
  ...
  for nsamples in samples:
    print('nsample size is:', nsamples)
    ...
    loc_loc_priors += [torch.tensor(ra.uniform(0.25,0.75,size=(nback,)),
      device = device), 
        torch.tensor(ra.uniform(0.25,0.75,size=nback), device = device)]
    ...
    # 8) Save some info
    np.savetxt("loss-history-{}.txt".format(nsamples), loss_hist)
