...
def time_base_strain(x, coeff1, coeff2):
    tiny = 1.0e-30
    B = np.sqrt(x + tiny) * coeff1
    A = np.log(x + tiny) * coeff2
...
def smooth_time(data):
    ...
    new_data = np.linspace(0, data[-1], data.shape[0])
    return new_data
...
if __name__ == "__main__":
    ...
    # coeffcients for random walking loading
    coeff1 = np.linspace(1.0, 10.0, 4)
    coeff2 = np.linspace(0.1, 10.0, 4)
    print(coeff1)
    ...
    nsamples = 5  # Number of repeats of each test
    ...
    for max_e in max_strains:
        for t_hold, cof1, cof2 in zip(tension_holds, coeff1, coeff2):
            ori_timesi, strainsi, cyclesi = experiments.sample_cycle_normalized_times(
                {
    ...
    for scale in scales:
        ...
        ds.to_netcdf("test-scale-%3.2f.nc" % scale)
