...
if __name__ == "__main__":
    ...
    input_data = xr.open_dataset(os.path.join("..", "sin-scale-%3.2f.nc" % scale))
    ...
    for i in range(data.shape[-1]):
        plt.plot(data[2, :, i].detach().numpy(), pred[:, i].detach().numpy(), label="Prediction")
        plt.plot(data[2, :, i].detach().numpy(), results[:, i].detach().numpy(), '--', label="True")
        plt.legend()
        plt.xlabel("Strain")
