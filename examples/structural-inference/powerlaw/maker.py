...
def power_model(E, n, eta, s0, A, p, scale_functions=None, device=torch.device("cpu"), **kwargs):
    """
    Key function for the entire problem: given parameters generate the model
    """
    if scale_functions is None:
        ...
        np_scale = scale_functions[5]

    isotropic = hardening.PowerlawHardeningModel(A, p, A_scale=A_scale, p_scale=np_scale)
    kinematic = hardening.NoKinematicHardeningModel()
    flowrule = flowrules.IsoKinViscoplasticity(
