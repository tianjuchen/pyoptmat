...
if __name__ == "__main__":
    ...
    if nsamples == 10:
        ics = torch.tensor(
            [0.49848774, 0.52056504, 0.47528703, 0.51003132, 0.45022538], device=device
        )
    elif nsamples == 20:
        ics = torch.tensor(
            [0.51255359, 0.50202772, 0.50684044, 0.51145004, 0.51277785], device=device
        )
    elif nsamples == 30:
        ics = torch.tensor(
            [0.48810693, 0.51449917, 0.51388876, 0.52200991, 0.51014777], device=device
        )
    ...
    print("Batch of %s  with score is %3.2f" % (nsamples, score))
