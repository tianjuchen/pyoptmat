...
def dist_box():
    ...
    variances = [0.05, 0.10, 0.15]

    mu_s5 = [0.49, 0.49, 0.49, 0.52, 0.49]
    std_s5 = [0.06, 0.06, 0.06, 0.07, 0.07]

    mu_s10 = [0.49, 0.49, 0.49, 0.52, 0.50]
    std_s10 = [0.07, 0.07, 0.07, 0.08, 0.07]

    mu_s15 = [0.51, 0.51, 0.50, 0.53, 0.50]
    std_s15 = [0.08, 0.08, 0.08, 0.09, 0.09]

    mu_bs = np.vstack((mu_s5, mu_s10, mu_s15))
    std_bs = np.vstack((std_s5, std_s10, std_s15))
    ...
    for i in range(mu_bs.shape[0]):
        ...
        for j in range(mu_bs.shape[-1]):
            pdist = dist.Normal(mu_bs[i, j], std_bs[i, j]).sample((amount,))
            p += list(pdist.numpy())
        tdist = dist.Normal(0.5, variances[i]).sample((amount,))
        ...
        ax.tick_params(width=3)
        plt.tight_layout()
        plt.savefig("variance-{}.pdf".format(variances[i]))
        # plt.show()
        plt.close()
    return names
