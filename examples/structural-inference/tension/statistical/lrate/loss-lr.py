...
def kl_param():
    ...
    lrates = ["learning rate 0.1", "learning rate 0.01", "learning rate 0.001"]
    ...
    for i in range(lrate_mean.shape[1]):
        for j in range(lrate_mean.shape[0]):
            ...
            score = z_test(lrate_mean[j, i], lrate_std[j, i])# * 1.0e6
    ...
    plt.legend(loc="upper left", frameon=False, ncol=1, prop={"size": 20})
    ...
    plt.tight_layout()
    plt.savefig("ztest-divergence-lrate.pdf")
    plt.show()
    plt.close()
    return names
...
def loss_plot():
    ...
    for fn, fp, lr in zip(fnames, fpath, lrs):
        ...
        plt.style.use(latex_style_times)
        plt.plot(df, lw=3, label="learning rate of {}".format(lr))
