...
if __name__ == "__main__":
    ...
    for amount in amounts:
        ...
        exp_data, exp_results, exp_cycles, exp_types, exp_control = downsample(
        ...
        indices = torch.randint(1, exp_results.shape[-1], (amount,))
        print(indices)
        data = exp_data[:, :, indices]
        results = exp_results[:, indices]
        cycles = exp_cycles[:, indices]
        types = exp_types[indices]
        control = exp_control[indices]
        print(data.shape)

        # 2) Setup names for each parameter and the priors
        names = ["n", "eta", "s0", "R", "d"]
        loc_loc_priors = [
            torch.tensor(0.15, device=device) for i in range(len(names))
        ]
        loc_scale_priors = [torch.tensor(0.15, device=device) for i in range(len(names))]
        scale_scale_priors = [torch.tensor(0.5, device=device) for i in range(len(names))]
        ...
        # 8) Plot convergence
        np.savetxt("loss-history-amount-{}.txt".format(amount), loss_hist)
