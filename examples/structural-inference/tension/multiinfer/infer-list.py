...
def make(p, **kwargs):
    ...
    """
    return make_model(torch.tensor(0.5), p[0], p[1], p[2], p[3], p[4], device=device, **kwargs).to(
        device
    )
...
if __name__ == "__main__":
    ...
    names = ["p"]
    loc_loc_priors = [
        torch.tensor(ra.random(5), device=device) for i in range(len(names))
    ]
    loc_scale_priors = [torch.tensor([0.15] * 5, device=device) for i in range(len(names))]
    scale_scale_priors = [torch.tensor([0.15] * 5, device=device) for i in range(len(names))]
    ...
    model = optimize.HierarchicalListModel(
