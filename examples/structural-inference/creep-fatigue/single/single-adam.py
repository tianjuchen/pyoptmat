...
if __name__ == "__main__":
    ...
    ics = [torch.tensor(ra.uniform(0.25, 0.25), device=device) for i in range(5)] + [
        torch.tensor(ra.uniform(0.25, 0.25, size=(3,)), device=device),
        torch.tensor(ra.uniform(0.25, 0.25, size=(3,)), device=device),
    ]
    ...
    np.savetxt("loss-history-single-adam.txt", loss_history)
    np.savetxt("time-adam.txt", time_list)
