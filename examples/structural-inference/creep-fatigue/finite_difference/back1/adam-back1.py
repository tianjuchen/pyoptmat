...
from maker import make_model, load_subset_data, make_model_1
...
if __name__ == "__main__":
    ...
    ics = [torch.tensor(ra.uniform(0.1, 0.1), device=device) for i in range(5)] + [
        torch.tensor(ra.uniform(0.1, 0.1, size=(1,)), device=device),
        torch.tensor(ra.uniform(0.1, 0.1, size=(1,)), device=device),
    ]
    ...
    np.savetxt("loss-pyoptmat-{}-{}-adam.txt".format(nsamples, scale), loss_history)
    np.savetxt("time-{}-{}-adam.txt".format(nsamples, scale), time_list)
