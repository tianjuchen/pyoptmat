...
class TestIsoKinPartialViscoplasticity(
    unittest.TestCase, CommonFlowRule, CommonFlowRuleBatchBatch
):
    def setUp(self):
        self.n = torch.tensor(5.2)
        ...
        self.model = flowrules.IsoKinPartialViscoplasticity(
            CP(self.n), CP(self.eta), CP(self.s0), 
            CP(self.alpha), CP(self.beta), CP(self.gamma), 
            self.iso, self.kin
        )
